...
" <test-running-functions>
  ...
  function! TerminalRun(cmd)
    execute "new"
    let s:test_buffer_number = bufnr('%')
    call termopen(a:cmd, {
  ...
  function! TerminalOnExit(job_id, exit_code, event) dict
    if a:exit_code == 0
      ...
      execute "b " s:test_buffer_number
      call search('[error]')
      wincmd =
    endif
  endfunction
...
function! KnitMarkdown()
  let l:pwd = expand('%:p:h')

  let l:outfile = l:pwd . "/index.html"
  let l:markdown = split(globpath(pwd, '*.Rmd'), '\n')[0]
  silent !clear

  if exists("l:markdown") 
	  silent execute "!Rscript -e 'rmarkdown::render(\"" . l:markdown  . "\", output_file=\"" . trim(l:outfile) . "\")'" 
  endif
endfunction

" If there are no errors just close the window
